...
load_image(std::filesystem::path p, int res_div) {
    ...
    if (file_c >= 3) {
        if (res_div <= 1) {
            ...
            if (!out) { in->close(); throw std::bad_alloc(); }
            ...
            if (!full) { in->close(); throw std::bad_alloc(); }
    ...
    // 1–2 channel inputs -> read native, then expand to RGB
    {
        ...
        if (in_c == 1) {
            ...
            for (size_t i = 0, N = (size_t)w * h; i < N; ++i) {
                unsigned char v = g[i];
                base[3*i + 0] = v; base[3*i + 1] = v; base[3*i + 2] = v;
            }
...
void save_image(const std::filesystem::path& path, torch::Tensor image) {
    ...
    if (image.dim() == 4) image = image.squeeze(0);               // [B,C,H,W] -> [C,H,W]
    if (image.dim() == 3 && image.size(0) <= 4) image = image.permute({1,2,0}); // [C,H,W]->[H,W,C]
    ...
    if (!out) {
        throw std::runtime_error("ImageOutput::create failed for "
                                 + fname + " : " + OIIO::geterror());
    }
    ...
    if (ext == ".jpg" || ext == ".jpeg") spec.attribute("CompressionQuality", 95);
...
void save_image(const std::filesystem::path& path,
                ...
                bool horizontal,
                int separator_width) {
    if (images.empty()) throw std::runtime_error("No images provided");
    if (images.size() == 1) { save_image(path, images[0]); return; }
    ...
    for (auto img : images) {
        ...
        if (img.dim() == 3 && img.size(0) <= 4) img = img.permute({1,2,0});
...
void BatchImageSaver::queue_save(const std::filesystem::path& path, torch::Tensor image) {
    if (!enabled_) { save_image(path, image); return; }
    SaveTask t;
    t.path = path;
    t.image = image.clone();
    t.is_multi = false;
    {
        std::unique_lock<std::mutex> lock(queue_mutex_);
        if (stop_) { save_image(path, image); return; }
...
void BatchImageSaver::queue_save_multiple(const std::filesystem::path& path,
                                          ...
                                          bool horizontal,
                                          int separator_width) {
    if (!enabled_) { save_image(path, images, horizontal, separator_width); return; }
    SaveTask t;
    t.path = path;
    t.images.reserve(images.size());
    for (const auto& img : images) t.images.push_back(img.clone());
    t.is_multi = true;
    ...
    {
        std::unique_lock<std::mutex> lock(queue_mutex_);
        if (stop_) { save_image(path, images, horizontal, separator_width); return; }
        task_queue_.push(std::move(t));
        active_tasks_++;
    }
    cv_.notify_one();
