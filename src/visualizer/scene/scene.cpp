...
namespace gs {
    ...
    int shN_coeffs = (stats.max_sh_degree > 0) ?
                     ((stats.max_sh_degree + 1) * (stats.max_sh_degree + 1) - 1) : 0;
    ...
    struct CombinedTensors {
        ...
        .shN = (shN_coeffs > 0) ?
               torch::zeros({static_cast<int64_t>(stats.total_gaussians), shN_coeffs, 3}, opts) :
               torch::empty({static_cast<int64_t>(stats.total_gaussians), 0, 3}, opts),
    ...
    for (const auto* model : visible_models) {
        ...
        if (shN_coeffs > 0) {
            ...
            int model_shN_coeffs = (model->shN().defined() && model->shN().dim() >= 2) ?
                                   model->shN().size(1) : 0;
