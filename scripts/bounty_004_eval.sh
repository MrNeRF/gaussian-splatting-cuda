...
# Per-scene max-cap from your table (commas removed)
declare -A MAX_CAPS=(
  [garden]=4937304
  [bicycle]=5684053
  [stump]=4647623
...
for SCENE in $SCENE_LIST; do
    ...
    # Scene-specific max cap (fallback to 1,000,000 if not in map)
    MAX_CAP=${MAX_CAPS[$SCENE]:-1000000}
    MAX_CAP_FMT=$(format_with_commas "$MAX_CAP")

    echo "========================================="
    echo "Running $SCENE with images_${DATA_FACTOR} (max-cap: $MAX_CAP_FMT)"
    echo "========================================="
