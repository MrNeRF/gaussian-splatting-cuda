...
struct PointCloud {
    torch::Tensor positions; // [N, 3] float32
    ...
    torch::Tensor features_dc;   // [N, 3, 1] float32
    torch::Tensor features_rest; // [N, 3, (sh_degree+1)^2-1] float32
    ...
    bool is_gaussian() const {
        return features_dc.defined() && features_dc.numel() > 0;
    }
    ...
    PointCloud to(torch::Device device) const {
        PointCloud pc;
        pc.positions = positions.defined() ? positions.to(device) : positions;
        pc.colors = colors.defined() ? colors.to(device) : colors;
        pc.normals = normals.defined() ? normals.to(device) : normals;
        pc.features_dc = features_dc.defined() ? features_dc.to(device) : features_dc;
        pc.features_rest = features_rest.defined() ? features_rest.to(device) : features_rest;
