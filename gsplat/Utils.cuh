...
template <class WarpT> inline __device__ void warpSum(int &val, WarpT &warp) {
...
template <class WarpT> inline __device__ void warpMax(int &val, WarpT &warp) {
    val = cg::reduce(warp, val, cg::greater<int>());
}

///////////////////////////////
// Hardware Reduce (requires Ampere)
// Adopted from https://github.com/MooreThreads/LiteGS
///////////////////////////////

template <class WarpT>
inline __device__ void AmpereWarpSum(float& data, WarpT& warp) {
    int exponent = (__float_as_uint(data) >> 23) & 0xff;
    warpMax(exponent, warp);
    exponent -= 127;
    ...
    warpSum(scaled_value, warp);

    data = scaled_value * valid * inv_scaler;
}

template <class WarpT>
inline __device__ void AmpereWarpSum(float2& data, WarpT &warp) {
    ...
    warpSum(scaled_value_x, warp);
    data.x = scaled_value_x * valid * inv_scaler;
    int scaled_value_y = static_cast<int>(data.y * scaler);
    warpSum(scaled_value_y, warp);
    data.y = scaled_value_y * valid * inv_scaler;
}

template <class WarpT>
inline __device__ void AmpereWarpSum(float3& data, WarpT &warp) {
    int exponent = (__float_as_uint(data.x) >> 23) & 0xff;
    exponent = max(exponent, (__float_as_uint(data.y) >> 23) & 0xff);
    ...
    warpSum(scaled_value_z, warp);
    data.z = scaled_value_z * valid * inv_scaler;
}

template <uint32_t DIM, class WarpT>
inline __device__ void AmpereWarpSum(float* val, WarpT& warp) {
    int max_exponent = INT_MIN;
#pragma unroll
    for (uint32_t i = 0; i < DIM; i++) {
        int exponent = (__float_as_uint(val[i]) >> 23) & 0xff;
        max_exponent = max(max_exponent, exponent);
    }
    warpMax(max_exponent, warp);
    max_exponent -= 127;

    int scale_exponent = 23 - max_exponent;
    bool valid = (max_exponent > -127) && (scale_exponent < 128);
...
#pragma unroll
    for (uint32_t i = 0; i < DIM; i++) {
        int scaled_value = static_cast<int>(val[i] * scaler);
        warpSum(scaled_value, warp);
        val[i] = scaled_value * valid * inv_scaler;
    }
}
